...
<?php
// Ambil return_date paling akhir
$result->data_seek(0); // reset pointer
$latestReturnDate = null;
while ($row = $result->fetch_assoc()) {
    if (!$latestReturnDate || $row['return_date'] > $latestReturnDate) {
        $latestReturnDate = $row['return_date'];
    }
}
?>
</table>
...
<form action="purchase.php" method="post">
    ...
    <input type="hidden" name="return_date" value="<?php echo $latestReturnDate; ?>">
    <button type="submit" class="btn" style="background:#11C9B6;border:none; ">Konfirmasi Pesanan</button>
</form>
