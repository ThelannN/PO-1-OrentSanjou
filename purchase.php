...
function dateDiffInDays($date1, $date2)
{
  ...
  return round(abs($timestamp1 - $timestamp2) / $oneDay);
}
...
if (isset($_POST['addCart']) && isset($_POST['return_date_cart']) && !empty($_POST['return_date_cart'])) {
  ...
  if ($numOfReturnDayCart > 0) {
    ...
    if (isset($_SESSION['id'])) {
      $sql22 = "INSERT INTO carts (
        pid, uid, product, price, quantity, return_date, rent_charge
      ) VALUES (
        {$_SESSION['book_id']},
        {$_SESSION['id']},
        '{$_SESSION['bt']}',
          ...
          <td style="padding: 10px;">
            <h5 style="margin: 0;position:relative">
              <div class="img-magnifier-container">
                <img id='image-pr' src="images/<?php echo $row['book_img'] ?>" style="height:90px;width:90px"
                  alt="product-img">
              </div>
              <br>
            ...
            <h4 style="margin: 0;">
              <form style='display:flex;gap:20px' action="<?php echo $_SERVER['PHP_SELF']; ?> " method="post">
                ...
                <div style="margin-top: 10px;">
                  <h6 class="section-title">Return Date For Cart</h6>
                  <input type="date" name="return_date_cart" id="dateInput" required>
                </div>

                <div style="margin-top: 15px;">
                  <input type='submit'  value='Add to Cart' name='addCart'></input>
                </div>
              </form>
            </h4>
          </td>
          ...
          <form method="POST">
            <tr>
              ...
              <td style="padding: 10px;">
                ...
                <h6 class="section-title" style='margin-bottom:5px'>I will return on (buy now):</h6>
                <input type="date" name="return_date" id="dateInput" required value="<?php echo $returnDate; ?>">
                <br><br>
                <hr>
                <br>
          ...
          if ($numOfReturnDay > 0):
            ...
            <div style='display:flex; flex-direction:column; gap:10px; margin-top:20px;'>
              ...
              <p style="margin: 0;font-size:13px;">
                Rent rate: <strong><?php echo $rate; ?>%</strong> <br>
                Return deadline: <strong><?php echo $numOfReturnDay; ?> days</strong>
              </p>
              <form action="message.php?id=<?php echo $row['book_id'] ?>" method="POST">
                <input type="hidden" name="amount" value="<?php echo $_SESSION['payment_amt']; ?>">
                <button type="submit"
                  style="border-radius:5px; background:seagreen; color:white; padding:10px; border:none; cursor:pointer;">
                  Confirm & Proceed
                </button>
              </form>
            </div>
          <?php else: ?>
            <p style='font-size:14px'>Warning: <span style='color:red;font-size:12px'>Please enter a valid return
                date.</span></p>
          <?php endif; endif; ?>
        </h6>
        </td>
