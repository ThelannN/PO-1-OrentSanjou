...
$sql = "SELECT * FROM  costumes WHERE book_id='{$_SESSION['book_id']}';";
    ...
    <table style="border-collapse: collapse; text-align: left;margin-top:24px ">
      ...
      <tbody>
          ...
          <td style="padding: 10px;">
            <h5 style="margin: 0;position:relative">
              <div class="img-magnifier-container">
                <img id='image-pr' src="admin/upload/<?php echo $row['book_img'] ?>" style="height:90px;width:90px"
                  alt="product-img">
              </div>
              <br>
        ...
        <tr>
          ...
          <td style="padding: 10px;">
            <p style="margin: 0;font-size:13px"><?php echo $row['book_price'] . " " . "(" . $row['book_type'] . ")" ?>
        ...
        <tr>
          ...
          <td style="padding: 10px;">
            <p style="margin: 0;font-size:13px"><?php echo $row['book_author'] ?></p>
          </td>
        </tr>
        <tr>
          <td style="padding: 10px; font-weight: bold;">
          ...
          <td style="padding: 10px;">
            <p style="margin: 0;font-size:13px;width:60%"><?php echo $row['book_desc'] ?></p>
          </td>
        <tr>
          <td style="padding: 10px; font-weight: bold;">
          ...
          <td style="padding: 10px;">
            <p style="margin: 0;font-size:13px"><?php echo $row['book_date'] ?></p>
          </td>
        </tr>
        </tr>
        <tr>
            ...
            <h4 style="margin: 0;">
              <form style='display:flex;gap:20px' action="<?php echo $_SERVER['PHP_SELF']; ?> " method="post">
                ...
                <div>
                  ...
                  <?php if ($numOfReturnDayCart <= 0)
                    echo "<p style='font-size:12px;margin-bottom:4px'>Warning: <span style='color:red;font-size:12px'>Please Enter Correct Date</span></p>"; ?>
                </div>
                <div class="btn-pr">
              ...
              <td style="padding: 10px;display:flex;">
                <div style="margin: 0;font-size:12px">
                  <p style="margin: 0;font-size:12px;margin-bottom:2px;">
                    <label for="payment_method" style="font-size:13px;margin-right:10px;">Select Payment Method:</label>
                    <select name="payment_method" id="payment_method" required style="font-size:13px;padding:3px 6px;">
                      <option value="" disabled selected>-- Choose one --</option>
                      <option value="transfer_bank_bni">Bank Transfer BNI (1860722837)</option>
                      <option value="ewallet">E-Wallet ( GoPay, Dana) (082188749315)</option>
                    </select>
                    <label for="image-post">Image</label>
                    <input style="width:50%;" id="bukti-img" name="bukti-img" type="file" required>
                  <div>
                    (Rent 1 Costume at a Time)
                    </p>
                    <button type='submit' name='pay'
                      style='border-radius:5px;background:skyblue;width:118px;height:30px;border:none;cursor:pointer;color:white'
                      id='#pay'>Proceed</button>
                  </div>

          </form>
          <?php
        } else
          echo '   <td style="padding: 10px; font-weight: bold;"><h5>Payment:</h5></td>
<td style="padding: 10px"><a href="./login.php" style="margin: 0;font-size:12px">Login First:</a>'
            ?>
          <?php
        if (isset($_POST['pay'])) {

          // Validasi apakah return_date dan payment_method diisi
          if (!empty($_POST['return_date']) && !empty($_POST['payment_method'])) {

            $_SESSION['return_date'] = $_POST['return_date'];
            $_SESSION['payment_method'] = $_POST['payment_method'];// simpan metode pembayaran
            if (isset($_FILES['bukti-img']) && $_FILES['bukti-img']['error'] === 0) {
              // Validasi ekstensi dan ukuran
              $allowed_exts = ['jpg', 'jpeg', 'png'];
              $file_name = $_FILES['bukti-img']['name'];
              $file_tmp = $_FILES['bukti-img']['tmp_name'];
              $file_size = $_FILES['bukti-img']['size'];
              $file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));

              if (in_array($file_ext, $allowed_exts) && $file_size <= 2097152) {
                $upload_path = "admin/upload/bukti/" . $file_name;
                move_uploaded_file($file_tmp, $upload_path);
                $_SESSION['bukti-img'] = $file_name;
              } else {
                echo "<p style='color:red;'>Invalid file type or file too large</p>";
              }
            }
            ...
            if ($numOfReturnDay > 0) {
              ...
              <div style='display:flex; flex-direction: column; gap: 10px; margin-top: 10px;'>
                <p style='color:crimson;margin-bottom:2px'>
                  <u>Read Carefully Before Proceeding</u>
                </p>
                <p style="margin: 0;font-size:12px;">
                  Rent rate according to selected date: <?php echo $rate . "%" ?><br>
                  You need to return the rented costume within: <?php echo $numOfReturnDay . " days"; ?><br>
                  Selected Payment Method:
                  <strong><?php echo ucwords(str_replace("_", " ", $_SESSION['payment_method'])) ?></strong>
                  Bukti Pembayaran:
                  <strong><?php echo ucwords(str_replace("_", " ", $_SESSION['bukti-img'])) ?></strong>
                </p>

                <!-- Form konfirmasi -->
                <form action="message.php?id=<?php echo $row['book_id'] ?>" method="POST">
                  <button type='submit'
                    style='border-radius:5px;background:green;width:150px;height:35px;border:none;cursor:pointer;color:white'>
              ...
              echo "<p style='font-size:14px'>Warning: <span style='color:red;font-size:12px'>Please enter a correct date.</span></p>";
            }

          } else {
            // Tampilkan pesan jika input kosong
            echo "<p style='font-size:14px'>Warning: <span style='color:red;font-size:12px'>Return date and payment method are required.</span></p>";
          }
        }
        ?>
        </h6>
